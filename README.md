...
- Docker Desktop + Docker Compose. Không cần cài Python ở host — mọi thứ chạy trong container.
...
Bật Kafka, Spark, PostgreSQL, MinIO, pgAdmin, Jupyter, **Streamlit**… và tạo 3 Kafka topic với
retention tương ứng.
...
| `streaming` | `setup.sh` → `pipeline` → nạp PostgreSQL → bật Spark validator + updater → bắn producer → mirror MinIO |
